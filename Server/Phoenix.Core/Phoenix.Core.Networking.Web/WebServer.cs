...
public abstract class WebServer
{
	...
	private void ListenThread()
	{
		...
		while (true)
		{
			...
			DebugSystem.Write("Socket Type " + socket.SocketType, null, DebugItemType.Info_Heavy);
			if (socket.Connected)
			{
				DebugSystem.Write($"\nClient Connected by Web Server!!\n==================\nCLient IP {socket.RemoteEndPoint}\n", null, DebugItemType.Info_Heavy);
				byte[] array = new byte[1024];
				int num2 = socket.Receive(array, array.Length, SocketFlags.None);
				string @string = Encoding.ASCII.GetString(array);
				if (@string.Substring(0, 3) != "GET")
				{
					DebugSystem.Write("Only Get Method is supported..", null, DebugItemType.Info_Heavy);
					socket.Close();
				}
				else
				{
					...
					else
					{
						...
						else
						{
							...
							text2 = text8 + text6;
							DebugSystem.Write("File Requested : " + text2, null, DebugItemType.Info_Heavy);
							if (!File.Exists(text2))
							{
								string text9 = "<H2>404 Error! File Does Not Exists...</H2>";
								SendHeader(text4, "", text9.Length, " 404 Not Found", socket);
								SendToBrowser(text9, socket);
								DebugSystem.Write(data, null, DebugItemType.Info_Heavy);
							}
							else
							{
								int num3 = 0;
