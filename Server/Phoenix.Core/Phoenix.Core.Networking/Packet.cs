...
public class Packet : IPacket
{
	...
	public static T ConvertfromFormat<T>(string packetFormat, params object[] list)
	{
		...
		{
			return (T)Convert.ChangeType(new Packet(enumerable, enumerable.Count()), typeof(T));
		}
		if (typeof(T) == typeof(OutgoingPacket))
		{
			return (T)Convert.ChangeType(new OutgoingPacket(new Packet(enumerable, enumerable.Count())), typeof(T));
		}
		return default(T);
	}
	...
	{
		return string.Format("[Packet Size- {0} Group- {1} Header- {2} Data- {3}]", m_buffer.Length, m_buffer[4] + " " + ((m_buffer.Length > 5) ? m_buffer[5] : 0), string.Join(",", m_buffer.Take(4)), string.Join(",", m_buffer.Skip(4)));
	}
}
